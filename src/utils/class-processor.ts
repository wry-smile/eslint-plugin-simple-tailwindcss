...
const removeImportantPrefix = (token: string) => {
  if (token.startsWith("!")) {
...
const collapseWhitespace = (value: string) =>
  value.trim().replace(WHITESPACE_RE, " ");

const stripVariants = (token: string) => {
  let depth = 0;
...
const compileGroupDefinitions = (
  definitions: TailwindGroupDefinition[],
...
const resolveGroupWeight = (
  token: string,
  compiledGroups: CompiledGroupDefinition[],
) => {
...
const normalizeToken = (
  token: string,
  compiledGroups: CompiledGroupDefinition[],
  cache: Map<string, NormalizedToken>,
): NormalizedToken => {
...
export const createClassFormatter = (
...
export const formatClassString = (
  value: string,
  config?: FormatterConfig,
): string | null => {
  if (!config) {
